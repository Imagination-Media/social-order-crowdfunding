...
class Check
{
    ...
    public function execute()
    {
        ...
        foreach ($collection as $item) {
            ...
            if (!is_array($incrementId) && !empty($incrementId)) {
                $price = (float)$item->getPrice();
                $qty = (int)$item->getQtyInvoiced() - (int)$item->getQtyRefunded();
                $currentPrice = (isset($orders[$incrementId])) ? $orders[$incrementId] : 0;
                $orders[$incrementId] = $currentPrice + ($price * $qty);
            }
        }

        foreach ($orders as $incrementId => $totalAmount) {
            ...
            if (count($orderList) > 0) {
                ...
                if (!$order->isCanceled() && !$order->hasInvoices()) {
                     ...
                     */
                    if ($totalAmount >= (float)$order->getGrandTotal()) {
                        /**
                         * Create invoice
                         */
